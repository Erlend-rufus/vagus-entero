...
// § 8. Denne vakten gjør LANSERING.md-punktet håndhevet i stedet for antatt.
...
export function kjorDist(distKatalog, { klinikkSti = 'src/_data/klinikk.json' } = {}) {
  ...
  return LANSERINGSKRITISKE.filter((felt) => !klinikk[felt]).map(
    (felt) =>
      `klinikk.json-feltet «${felt}» er tomt i et ekte produksjonsbygg — ehandelsloven § 8 krever det i bunnteksten`
  );
}
