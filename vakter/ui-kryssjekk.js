...
export function kjorKilde() {
  const feil = [];
  const nokler = new Set(Object.keys(JSON.parse(fs.readFileSync('src/_data/ui.json', 'utf8'))));
  ...
  for (const fil of finnRepoTekstfiler()) {
    ...
    for (const m of lesTekst(fil).matchAll(/(?<![\p{L}\p{N}_.])ui\.([a-zA-Z_][a-zA-Z0-9_]*)/gu)) {
      brukte.add(m[1]);
      if (!nokler.has(m[1])) {
